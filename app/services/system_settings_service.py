...
class BotConfigurationService:
    ...
    CATEGORY_KEY_OVERRIDES: dict[str, str] = {
        ...
        'PENALTY_SQUAD_ENABLED': 'SQUAD_PENALTY',
        'PENALTY_SQUAD_UUID': 'SQUAD_PENALTY',
    ...
    SETTING_HINTS: dict[str, dict[str, str]] = {
        ...
        'PENALTY_SQUAD_ENABLED': {
            'description': 'Включить систему штрафного сквада для пользователей, заблокировавших бота.',
            'format': 'Булево значение.',
            'example': 'true',
            'warning': 'Для работы также нужно указать PENALTY_SQUAD_UUID и DEFAULT_SQUAD_UUID.',
        },
        'PENALTY_SQUAD_UUID': {
            'description': 'UUID штрафного сквада в Remnawave — урезанный сервер для нарушителей.',
            'format': 'UUID сквада из Remnawave.',
            'example': 'xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx',
            'warning': 'Сквад должен существовать в Remnawave. Пользователь получит этот сервер вместо обычного.',
        },
        'DEFAULT_SQUAD_UUID': {
            'description': 'UUID обычного (дефолтного) сквада — куда возвращать пользователя после разблокировки.',
            'format': 'UUID сквада из Remnawave.',
            'example': 'xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx',
            'warning': 'Если не указан, при возврате у пользователя будет очищен externalSquadUuid.',
        },
        'PENALTY_AUTO_SCAN_ENABLED': {
            'description': 'Автоматически сканировать подписчиков и перемещать новых нарушителей в штрафной сквад.',
            'format': 'Булево значение.',
            'example': 'true',
            'dependencies': 'PENALTY_SQUAD_ENABLED=true',
        },
        'PENALTY_AUTO_SCAN_INTERVAL_HOURS': {
        ...
        'MODEM_ENABLED': {
            'description': 'Включает функцию модема — дополнительное устройство к подписке за отдельную плату.',
            'format': 'Булево значение.',
            'example': 'true',
            'warning': 'При включении пользователи увидят секцию модема в кабинете.',
        },
        'MODEM_PRICE_PER_MONTH': {
            'description': 'Цена модема за месяц в копейках. Итоговая цена рассчитывается подневно.',
            'format': 'Целое число в копейках (10000 = 100₽).',
            'example': '10000',
        },
        'MODEM_PERIOD_DISCOUNTS': {
            'description': 'Скидки на модем за длительный срок. Формат: "месяцев:процент".',
            'format': 'Через запятую: "3:15,6:20,12:25".',
