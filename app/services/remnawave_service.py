...
async def get_panel_user_ref(
    ...
    *,
    ...
    """Возвращает (None, remna_id) для идентификации пользователя в панели.

    3.0.0 полностью выпилил панельный uuid — идентичность пользователя чисто
    числовая (``remnawave_id``). Первый элемент кортежа сохранён по историческим
    причинам (много вызывающих кодов распаковывают ``_uuid, _remna_id = ...``),
    но всегда ``None``: ``users.remnawave_uuid``/``subscriptions.remnawave_uuid``
    — мёртвые исторические колонки, читает их только одноразовый бэкфилл.

    Если remnawave_id уже есть — возвращает (None, id) немедленно. Иначе
    пытается разрешить через short_uuid (из subscription или
    user.subscriptions[0]), потом через telegram_id. Найденный id
    сохраняется в user.remnawave_id и коммитится в db (best-effort).
    """
    ...
    if short_uuid:
        resolved = await client.resolve_user(short_uuid=short_uuid)
        if resolved:
    ...
    if resolved_id is None and user is not None and getattr(user, 'telegram_id', None):
        panel_users = await client.find_users_by_telegram_id(user.telegram_id)
        if panel_users:
