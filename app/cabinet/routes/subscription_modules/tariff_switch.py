...
async def switch_tariff(
    ...
    try:
        ...
        async with service.get_api_client() as api:
            ...
            _use_sub = subscription if (settings.is_multi_tariff_enabled() and subscription and subscription.remnawave_uuid) else None
            _sw_uuid, _sw_id = await get_panel_user_ref(api, db, user=user, subscription=_use_sub)
            if _sw_uuid or _sw_id:
                await api.reset_user_devices(uuid=_sw_uuid, remna_id=_sw_id)
                devices_reset = True
