...
async def purchase_tariff(
    ...
    try:
        ...
        try:
            ...
            # В classic mode тариф не привязывается к подписке (используется только для
            # пометки триала через is_trial_available). Передавать tariff_id = id триал-тарифа
            # = подписка остаётся 'на пробном тарифе' после конверсии (UI показывает пробный,
            # докупка устройств блокируется). В classic mode tariff_id = None — чистая платная.
        ...
        try:
            ...
            dispatch_subscription_purchase_notification_bg(
                ...
                transaction_id=transaction.id if transaction else None,
                period_days=period_days,
                # Маркер ставит extend_subscription, когда покупка конвертировала
                # живой триал (в т.ч. конверсию внутри create_paid_subscription).
...
async def activate_trial(
    ...
    # Активация ядра: вычисление параметров триала, создание подписки,
    # синхронизация Remnawave и отправка административных уведомлений.
