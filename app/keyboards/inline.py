...
def get_ticket_view_keyboard(
    ...
    if not is_closed and show_call_operator:
        keyboard.append(
            [
                InlineKeyboardButton(
                    text=texts.t('CALL_OPERATOR', '👨‍💼 Позвать оператора'),
                    callback_data=f'request_operator:{ticket_id}',
                )
            ]
        )
