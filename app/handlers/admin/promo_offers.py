...
async def _send_offer_to_users(
    ...
    *,
    ...
    # Ограничение на количество одновременных отправок. Это же число — залп
    # одновременных подключений к базе: каждая отправка открывает свою сессию.
    # На двадцати параллельных резолв имени хоста через встроенный DNS докера
    # начинал терять UDP-запросы, и отправка падала таймаутом соединения.
    semaphore = asyncio.Semaphore(max(1, settings.PROMO_BROADCAST_CONCURRENCY))
