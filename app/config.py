...
class Settings(BaseSettings):
    ...
    ADMIN_NOTIFICATIONS_WHEEL_TOPIC_ID: int | None = None  # Колесо удачи: спины, выигрыши
    ADMIN_NOTIFICATIONS_FALLBACK_TOPIC_ID: int | None = None  # Переезды в fallback-сквад
    ...
    ADMIN_NOTIFICATIONS_WHEEL_ENABLED: bool = True
    ADMIN_NOTIFICATIONS_FALLBACK_ENABLED: bool = True
    ...
    # Сколько отправок промо-рассылки идёт одновременно. Каждая открывает СВОЮ
    # сессию к базе, поэтому число — это ещё и залп одновременных подключений.
    # Узкое место рассылки — лимиты Telegram, а не база, так что держим скромно:
    # на двадцати параллельных подключениях встроенный DNS докера начинал терять
    # запросы, и отправки падали таймаутом соединения.
    PROMO_BROADCAST_CONCURRENCY: int = 5
    ...
    #
    # ВАЖНО: значение должно быть >= CART_TTL_SECONDS. Иначе возникает «мёртвая зона»:
    # корзина ещё жива (get_user_cart отдаёт её до CART_TTL), автопокупка запускается,
    # но флаг намерения уже истёк → has_topup_intent=False → тихий пропуск (деньги
    # списаны, подписка не куплена). Медленная оплата (банк/3DS/крипта 30-60 мин)
    # попадала ровно в эту дыру. «Забытую» корзину по-прежнему ограничивает CART_TTL,
    # т.к. сама корзина мрёт через час, а флаг гасится только при успешной покупке.
    CART_AUTOPURCHASE_INTENT_TTL_SECONDS: int = 3600  # = CART_TTL_SECONDS (1 час)
    ...
    # Массовые письма (промо-рассылки) шлём заметно реже транзакционных. Исходящий
    # антиспам Яндекса режет за темп и однотипность: в проде он отбивал каждое
    # второе письмо на yandex.ru и mail.ru, пропуская gmail и vk. Разный интервал
    # нужен ещё и затем, чтобы код подтверждения входа не стоял в очереди за
    # рассылкой — человек не должен не попадать в кабинет из-за промо-акции.
    SMTP_BULK_MIN_SEND_INTERVAL_MS: int = 6000
    # Пауза после отказа «письмо похоже на спам»: продолжать в том же темпе
    # значит добивать репутацию отправителя.
    SMTP_SPAM_REJECT_COOLDOWN_S: int = 60
