...
class TestRedisPayloadFunctions:
    ...
    async def test_save_pending_payload_to_redis_success(self, monkeypatch):
        ...
        with patch.object(channel_checker.cache, 'set', new_callable=AsyncMock) as mock_set:
    ...
    async def test_get_pending_payload_from_redis_success(self, monkeypatch):
        ...
        with patch.object(channel_checker.cache, 'get', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = 'ref_test123'
    ...
    async def test_delete_pending_payload_from_redis(self, monkeypatch):
        ...
        with patch.object(channel_checker.cache, 'delete', new_callable=AsyncMock) as mock_delete:
...
class TestPayloadIntegration:
    ...
    async def test_payload_retrieved_from_redis_fallback(self, monkeypatch):
        ...
        with patch.object(
            __import__('app.middlewares.channel_checker', fromlist=['cache']).cache,
            'get',
            new_callable=AsyncMock,
        ) as mock_get:
            mock_get.return_value = 'ref_from_redis'
